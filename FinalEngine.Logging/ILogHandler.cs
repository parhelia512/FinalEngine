namespace FinalEngine.Logging
{
    ...
    /// </summary>
    /// <remarks>
    ///   <para>An <see cref="ILogHandler"/> provides a way for users to extend the API and allow information to be logged to other destinations (such as a database or the console). An <see cref="ILogHandler"/> provides a single method; <see cref="ILogHandler.Log(LogType, string)"/> that user must implement.</para>
